...
class MdNumberSelection extends StatefulWidget {
  const MdNumberSelection({
    super.key,
    this.initialValue,
    this.onChanged,
  ...
  /// Theme of the [MdNumberSelection] widget:
  ///
  ///
  ...
  final MdNumberSelectionTheme? theme;

  @override
  State<MdNumberSelection> createState() => _MdNumberSelectionState();
}

class _MdNumberSelectionState extends State<MdNumberSelection>
  ...
  late MdNumberSelectionTheme _theme;
  ...
  void _changeValue({required bool adding, bool fromButtons = false}) async {
    ...
    if (adding && _value + widget.increment <= widget.maxValue) {
      setState(() {
        _value = _value + widget.increment;
      });
    ...
    if (widget.withSpring) {
      final kDefaultSpring = SpringDescription.withDampingRatio(
